...
#define ABS(N) ((N) >= 0 ? (N) : -(N))
  ...
  struct CalendarDateTime_t {
    uint16_t gpsYear;   //!< Year ( year > 1900 )
    uint8_t gpsMonth;   //!< Month ( 1 <= month <= 12 )
    uint8_t gpsDay;     //!< Day ( 1 <= day <= 31 )
    uint8_t gpsHour;    //!< Hour ( 0 <= hour <= 23 )
    uint8_t gpsMinute;  //!< Minute ( 0 <= minute <= 59 )
    uint8_t gpsSecond;  //!< Second ( 0 <= second <= 59 )
  };
